...
class RodManager:
    ...
    def remove_rod_from_hand(self, slot: int) -> bool:
        ...
        try:
            ...
            _safe_print(f"\n🎣 [ROD_MANAGER] === REMOVENDO VARA SLOT {slot} ===")

            if not self.input_manager:
                _safe_print("❌ InputManager não disponível")
                return False

            _safe_print(f"   📊 InputManager tipo: {type(self.input_manager).__name__}")
            _safe_print(f"   📊 Tem press_key? {hasattr(self.input_manager, 'press_key')}")
            ...
            _safe_print(f"   📞 Chamado por: {caller}")
            _safe_print(f"   🎯 Ação: REMOVER vara da mão")
            _safe_print(f"   🚀 Enviando press_key('{slot}', duration=0.05)...")

            result = self.input_manager.press_key(str(slot), duration=0.05)  # Clique rápido!
        ...
        try:
            with self.rod_lock:
                ...
                if vara1_usos < limite and vara2_usos >= limite:
                    ...
                    if vara1_usos >= limite and vara2_usos >= limite:
                        _safe_print(f"\n⚠️ [PAR ESGOTADO] AMBAS as varas atingiram limite de {limite} usos!")
                        _safe_print(f"   Vara {vara1_slot}: {vara1_usos}/{limite} usos")
                        _safe_print(f"   Vara {vara2_slot}: {vara2_usos}/{limite} usos")
                        _safe_print(f"   🔄 Trocando para PRÓXIMO PAR...")

                        # ✅ TROCAR PARA PRÓXIMO PAR
                        self.current_pair_index = (self.current_pair_index + 1) % len(self.rod_pairs)
                        next_pair = self.rod_pairs[self.current_pair_index]
                        _safe_print(f"   ✅ Novo par: {next_pair}")

                        # ✅ RESETAR contadores do novo par (iscas foram reabastecidas na manutenção)
                        self.rod_uses[next_pair[0]] = 0
                        self.rod_uses[next_pair[1]] = 0
                        _safe_print(f"   🔄 Contadores resetados: Vara {next_pair[0]}: 0/{limite}, Vara {next_pair[1]}: 0/{limite}")

                        # Equipar PRIMEIRA vara do novo par
                        next_rod = next_pair[0]
                        next_rod_in_pair = 0
                        _safe_print(f"   ✅ Equipando vara {next_rod} do novo par\n")
    ...
    def register_rod_use(self, rod: Optional[int] = None, caught_fish: bool = True, will_open_chest: bool = False):
        """
        📊 Registrar uso de uma vara (SIMPLIFICADO - sem lógica de decisão)

        Args:
            rod: Número da vara (None = vara atual)
            caught_fish: True se pegou peixe, False se deu timeout
            will_open_chest: Ignorado (mantido para compatibilidade)

        ✅ SERVIDOR decide quando trocar par!
        """
        try:
            if rod is None:
            ...
            _safe_print(f"📊 {status} - Vara {rod}: {self.rod_uses[rod]} usos")

            # ✅ NÃO DECIDE! Apenas retorna False (sem troca de par)
            # Servidor vai decidir quando trocar baseado em rod_uses
            return False
    ...
    def execute_full_maintenance(self, chest_already_open: bool = False) -> bool:
        ...
        Args:
            chest_already_open: Se True, pula abertura/fechamento do baú
        """
        _safe_print("🔧 RodManager: Delegando para sistema de manutenção completo...")
        return self.maintenance_system.execute_full_maintenance(chest_already_open=chest_already_open)
