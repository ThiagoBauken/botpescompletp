...
# Sistema de criptografia para proteção de dados
try:
...
class LicenseManager:
    ...
    def __init__(self):
        ...
        # Inicializar crypto manager para proteção de dados
        if CRYPTO_AVAILABLE:
            self.crypto = CryptoManager()
            _safe_print("🔐 Sistema de criptografia ativado")
        else:
            self.crypto = None
    ...
    def load_license(self) -> Optional[str]:
        ...
        Tenta descriptografar se crypto disponível.
        Fallback para plaintext se falhar (compatibilidade).
        """
        try:
            if os.path.exists(self.license_file):
                ...
                # Tentar descriptografar se crypto disponível
                if self.crypto:
                    # Verificar se parece ser base64 (licença criptografada)
                    # Base64 válido tem comprimento múltiplo de 4 e apenas caracteres válidos
                    import re
                    is_base64_like = (
                        len(stored_data) % 4 == 0 and
                        re.match(r'^[A-Za-z0-9+/]*={0,2}$', stored_data) is not None
                    )
                    ...
                    if is_base64_like:
                        try:
                            ...
                            # Erro na descriptografia, mas parecia base64
                            _safe_print(f"⚠️ Erro ao descriptografar (tentando plaintext): {e}")
                            return stored_data
                    else:
                        # Não parece base64, assumir plaintext (licença antiga)
                        _safe_print("⚠️ Licença em formato antigo (plaintext)")
                        return stored_data
                else:
                    # Sem crypto, retornar plaintext
                    return stored_data
    ...
    def save_license(self, key: str) -> bool:
        ...
        Criptografa a licença se crypto disponível.
        Caso contrário, salva em plaintext.
        """
        try:
            # Criptografar se crypto disponível
            if self.crypto:
                encrypted_key = self.crypto.encrypt(key)
                with open(self.license_file, 'w', encoding='utf-8') as f:
                    f.write(encrypted_key)
                _safe_print("💾 Licença salva e criptografada com sucesso!")
            else:
                # Fallback: salvar em plaintext
                with open(self.license_file, 'w', encoding='utf-8') as f:
                    f.write(key)
    ...
    def check_license(self) -> bool:
        ...
        if saved_key:
            ...
            if valid:
                ...
                # IMPORTANTE: Setar estado interno após validação
                self.licensed = True
