...
class InventoryManager:
    ...
    def should_trigger_cleaning(self) -> bool:
        ...
        # ✅ PROTEÇÃO: Não permitir limpeza se última tentativa foi há menos de 5 segundos
        time_since_last_attempt = time.time() - self.last_cleaning_attempt
        ...
        if cleaning_mode == CleaningMode.AUTO_INTERVAL.value:
            # Baseado em quantidade de pescas
            interval = config['auto_clean_interval']
            return self.fish_count_since_cleaning >= interval
    ...
    def increment_fish_count(self):
        ...
        with self.cleaning_lock:
            ...
            _safe_print(f"🐟 [CLEANING] Contador incrementado: {self.fish_count_since_cleaning} peixes desde última limpeza")

            # ✅ LOG: Verificar configuração atual
            config = self.get_cleaning_config()
            ...
            # ✅ LOG: Verificar se vai triggar
            should_trigger = self.should_trigger_cleaning()
            if should_trigger:
                _safe_print(f"✅ [CLEANING] TRIGGER ATIVO! Limpeza será executada no próximo ciclo")
            else:
                _safe_print(f"⏳ [CLEANING] Ainda não atingiu threshold (precisa {interval}, tem {self.fish_count_since_cleaning})")
    ...
    def _execute_fish_transfer(self) -> bool:
        ...
        try:
            ...
            transferred_count = 0
            max_scan_attempts = 10  # Evitar loop infinito
            scan_attempt = 0

            _safe_print("🔍 Iniciando limpeza dinâmica com re-escaneamento...")
            ...
            while scan_attempt < max_scan_attempts:
                ...
                for i, (fish_name, position) in enumerate(fish_to_transfer):
                    ...
                    if self._transfer_item_to_chest(position, max_attempts):
                        transferred_count += 1
                ...
                if transferred_in_batch == len(fish_to_transfer):
                    ...
                    time.sleep(0.5)  # Pausa para garantir que itens foram processados
                else:
                    break
        ...
        try:
            ...
            for fish_name in self.fish_templates.keys():
                if not self.template_engine.has_template(fish_name):
                    _safe_print(f"⚠️ [DEBUG] Template {fish_name} NÃO encontrado no cache")
                    continue

                # Buscar múltiplas ocorrências deste template
                _safe_print(f"🔍 [DEBUG] Detectando {fish_name}...")
                multiple_results = self._detect_multiple_occurrences_raw(fish_name, screenshot)
                _safe_print(f"   [DEBUG] {fish_name}: {len(multiple_results)} detecções RAW")
                ...
                if len(multiple_results) > 0:
                    _safe_print(f"   🐛 [DEBUG] {fish_name}: {in_inventory}/{len(multiple_results)} detecções no inventário (baú excluído)")
            ...
            _safe_print(f"🎯 Total de detecções brutas (peixes + iscas): {len(all_detections)}")

            # 🐛 DEBUG: Mostrar todas as detecções brutas
            _safe_print(f"🐛 [DEBUG] Detecções brutas completas:")
            for det in all_detections[:10]:  # Mostrar primeiras 10
                _safe_print(f"     {det['template']} em ({det['x']}, {det['y']}) - conf:{det['confidence']:.3f} - grupo:{det['group']}")

            # ✅ NMS POR GRUPOS (Task 1)
            group_filtered = []
            _safe_print(f"🐛 [DEBUG] Iniciando NMS por grupos...")
            for group_name, templates in template_groups.items():
                group_detections = [d for d in all_detections if d['group'] == group_name]

                if not group_detections:
                    _safe_print(f"  🐛 [DEBUG] Grupo '{group_name}': 0 detecções - pulando")
                    continue

                _safe_print(f"🔄 Grupo '{group_name}': {len(group_detections)} detecções - aplicando NMS...")
    ...
    def _detect_multiple_occurrences_raw(self, template_name: str, screenshot) -> List[Tuple[int, int, float, int, int]]:
        ...
        try:
            ...
            if template_clean in ['salmonn', 'troutt']:
                ...
                _safe_print(f"   🎯 [DEBUG] {template_name}: threshold {old_threshold:.2f} → {confidence_threshold:.2f}")
