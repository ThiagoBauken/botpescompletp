...
class ArduinoInputManager:
    ...
    def _send_command(self, command: str, timeout: float = 1.0) -> Optional[str]:
        ...
        with self.lock:
            if not self.connected or not self.serial:
                _safe_print("⚠️ Arduino não conectado")
                return None
            ...
            try:
                ...
                _safe_print(f"⚠️ Timeout aguardando resposta para: {command}")
                return None
        ...
        try:
            ...
            if success:
                ...
                try:
                    ...
                    _safe_print(f"   ❌ [PRESS_KEY] Force release FALHOU!")
    ...
    def mouse_down(self, button: str = 'left') -> bool:
        ...
        if PYAUTOGUI_AVAILABLE:
            ...
            _safe_print(f"")
            _safe_print(f"🔍 [MOUSE_DOWN] DEBUG MOVIMENTO:")
            _safe_print(f"   📍 Posição ANTES: ({pos_before.x}, {pos_before.y})")

        # ✅ CORRIGIDO: Enviar "left" ou "right" completo (Arduino espera isso!)
        _safe_print(f"   📤 Enviando: MOUSE_DOWN:{button}")
        ...
        if PYAUTOGUI_AVAILABLE:
            ...
            _safe_print(f"   📍 Posição DEPOIS: ({pos_after.x}, {pos_after.y})")
            delta_x = pos_after.x - pos_before.x
            delta_y = pos_after.y - pos_before.y
            if delta_x != 0 or delta_y != 0:
                _safe_print(f"   🚨 MOVIMENTO DETECTADO: ({delta_x:+d}, {delta_y:+d}) pixels!")
            else:
                _safe_print(f"   ✅ SEM MOVIMENTO (delta: 0, 0)")
            _safe_print(f"")
    ...
    def mouse_down_relative(self, button: str = 'left') -> bool:
        ...
        """
        _safe_print(f"🎯 [REL] Pressionando botão {button} (Mouse relativo)...")
        _safe_print(f"   📤 Enviando: MOUSE_DOWN_REL:{button}")
        response = self._send_command(f"MOUSE_DOWN_REL:{button}")
        _safe_print(f"   📥 Arduino respondeu: {response}")
        success = response and response.startswith("OK")
        ...
        if success:
            if button == 'left':
                ...
                self.mouse_state['right_button_down'] = True
            _safe_print(f"✅ [REL] Botão {button} pressionado - Estado atualizado: right_down={self.mouse_state.get('right_button_down', False)}")
        else:
    ...
    def drag(self, start_x: int, start_y: int, end_x: int, end_y: int, duration: float = 1.0) -> bool:
        ...
        try:
            _safe_print(f"\n{'#'*60}")
            _safe_print(f"🖱️ [DRAG] INÍCIO DO ARRASTO")
            _safe_print(f"   Origem: ({start_x}, {start_y})")
            _safe_print(f"   Destino: ({end_x}, {end_y})")
            _safe_print(f"   Duração: {duration}s")
            _safe_print(f"{'#'*60}\n")

            # PASSO 1: Mover para posição inicial
            _safe_print(f"   📍 [PASSO 1] Movendo para posição inicial ({start_x}, {start_y})...")
            if not self.move_to(start_x, start_y):
                _safe_print("   ❌ [PASSO 1] FALHA ao mover para posição inicial!")
                return False
            _safe_print(f"   ✅ [PASSO 1] Mouse em ({start_x}, {start_y})")
            time.sleep(0.2)

            # PASSO 2: Segurar botão esquerdo
            _safe_print(f"   🖱️ [PASSO 2] Pressionando botão esquerdo...")
            if not self.mouse_down('left'):
                _safe_print("   ❌ [PASSO 2] FALHA ao pressionar botão esquerdo!")
                return False
            _safe_print(f"   ✅ [PASSO 2] Botão esquerdo pressionado")
            time.sleep(0.2)

            # PASSO 3: Mover para destino COM botão pressionado
            # ✅ USAR MOUSEABS (absoluto) EM VEZ DE RELATIVO!
            _safe_print(f"   ➡️ [PASSO 3] Arrastando para ({end_x}, {end_y}) com duração {duration}s...")

            # ✅ CORREÇÃO CRÍTICA: Usar MOUSEABS para ir direto ao destino
            # Movimento relativo em loop causa o mouse ir até final da tela!
            _safe_print(f"   📍 [PASSO 3] Movendo diretamente para destino via MOUSEABS...")

            success = self.move_to(end_x, end_y)

            if not success:
                _safe_print("   ❌ [PASSO 3] FALHA ao mover para destino!")
                return False

            _safe_print(f"   ✅ [PASSO 3] Movimento via Arduino concluído")
            time.sleep(0.4)  # CRÍTICO: aguardar item chegar ao destino

            # PASSO 4: Soltar botão
            _safe_print(f"   🖱️ [PASSO 4] Soltando botão esquerdo...")
            if not self.mouse_up('left'):
                _safe_print("   ❌ [PASSO 4] FALHA ao soltar botão esquerdo!")
                return False
            _safe_print(f"   ✅ [PASSO 4] Botão esquerdo solto")
            ...
            _safe_print(f"✅ [DRAG] SUCESSO! Item arrastado para ({end_x}, {end_y})")
            return True

        except Exception as e:
            _safe_print(f"❌ [DRAG] ERRO CRÍTICO: {e}")
            import traceback
            traceback.print_exc()
            # CRÍTICO: Garantir que mouse não fique pressionado
            try:
                _safe_print("   🛑 [DRAG] Tentando soltar botão esquerdo...")
                self.mouse_up('left')
            except:
                pass
            return False
