...
class RodMaintenanceSystem:
    ...
    def execute_full_maintenance(self, chest_already_open: bool = False) -> bool:
        ...
        try:
            with self.maintenance_lock:
                ...
                # ✅ CRÍTICO: Durante manutenção oportunística (após feeding/cleaning),
                # dar MAIS tempo para interface estabilizar completamente!
                wait_time = 1.5 if chest_already_open else 0.5
    ...
    def _save_to_chest_rightclick_v3_exact(self, det_x: int, det_y: int):
        ...
        if self.input_manager:
            ...
            time.sleep(0.8)  # Aguardar vara ir para o baú
    ...
    def _drag_to_discard_area(self, from_x: int, from_y: int):
        ...
        if self.input_manager:
            ...
            # Usar pyautogui diretamente como no v3 (mais confiável)
            import pyautogui
            pyautogui.mouseDown(button='left')
            time.sleep(0.3)
    ...
    def _save_to_chest_rightclick(self, slot_x: int, slot_y: int):
        ...
        if self.input_manager:
            ...
            # Retornar para vara quebrada e clicar direito (lógica v3)
            self.input_manager.move_to(slot_x, slot_y)
            time.sleep(0.3)
            self.input_manager.right_click(slot_x, slot_y)
            # Vara vai automaticamente para o baú
    ...
    def _execute_drag_drop_bait_v3_exact(self, bait_x: int, bait_y: int, slot_x: int, slot_y: int, slot: int) -> bool:
        ...
        try:
            # Desabilitar fail-safe temporariamente (igual V3)
            import pyautogui
    ...
    def _save_maintenance_screenshot(self, stage_name: str):
        ...
        try:
            ...
            screenshots_dir = "fishing_bot_v4/screenshots/maintenance"
    ...
    def _scan_chest_for_baits(self) -> List[Dict]:
        ...
        # Config usa: crocodilo, carneurso, carnedelobo, TROUTT, grub, minhoca
        template_to_config = {
            'carneurso': 'carneurso',
        ...
        # Templates de iscas
        bait_templates = ['crocodilo', 'carneurso', 'carnedelobo', 'TROUTT', 'grub', 'minhoca']
