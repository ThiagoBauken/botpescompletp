...
class FeedingSystem:
    ...
    def should_trigger_feeding(self) -> bool:
        """Verificar se deve executar alimentação"""
        config = self.get_feeding_config()
        trigger_mode = config['trigger_mode']
        
        if trigger_mode == TriggerMode.TIME_BASED.value:
            # Baseado em tempo
            minutes_passed = (time.time() - self.last_feeding_time) / 60
            trigger_minutes = config['trigger_minutes']
            return minutes_passed >= trigger_minutes
            
        elif trigger_mode == TriggerMode.CATCH_BASED.value:
            # Baseado em pescas
            trigger_catches = config['trigger_catches']
            return self.fish_count_since_feeding >= trigger_catches
    ...
    def increment_fish_count(self):
        ...
        with self.feeding_lock:
            ...
            _safe_print(f"🐟 [FEEDING] Contador incrementado: {self.fish_count_since_feeding} peixes desde última alimentação")
            ...
            _safe_print(f"📊 [FEEDING] Config: mode={trigger_mode}, trigger_catches={trigger_catches}")

            # ✅ LOG: Verificar se vai triggar
            should_trigger = self.should_trigger_feeding()
            if should_trigger:
                _safe_print(f"✅ [FEEDING] TRIGGER ATIVO! Alimentação será executada no próximo ciclo")
            else:
                _safe_print(f"⏳ [FEEDING] Ainda não atingiu threshold (precisa {trigger_catches}, tem {self.fish_count_since_feeding})")
