/*
 * ============================================================================
 * 🎮 ULTIMATE FISHING BOT - ARDUINO HID CONTROLLER v5.4 - BOOTKEYBOARD FIX
 * ============================================================================
 *
 * ✅ SOLUÇÃO: BootKeyboard (HID-Project) + AbsoluteMouse (HID-Project)
 *
 * MUDANÇA: Trocar NKROKeyboard → BootKeyboard
 *
 * POR QUÊ BOOTKEYBOARD?
 * - Mais simples que NKROKeyboard (menos sobrecarga)
 * - Pode ter MENOS CONFLITO com AbsoluteMouse
 * - Suporta 6 teclas + modificadores (suficiente para o bot)
 * - Compatível com BIOS (não precisamos, mas é bonus)
 *
 * BIBLIOTECAS USADAS:
 * - HID-Project: BootKeyboard + AbsoluteMouse + Mouse
 *
 * INSTALAÇÃO:
...
void checkKeysStuck() {
  ...
  if (tabPressed && (millis() - tabPressTime > TAB_AUTO_RELEASE_TIMEOUT)) {
    ...
    BootKeyboard.release(KEY_TAB);  // ← BOOTKEYBOARD
    tabPressed = false;
  }
  ...
  if (altPressed && (millis() - altPressTime > ALT_AUTO_RELEASE_TIMEOUT)) {
    ...
    BootKeyboard.release(KEY_LEFT_ALT);  // ← BOOTKEYBOARD
    altPressed = false;
  }
}
...
void setup() {
  ...
  // ✅ USAR BOOTKEYBOARD (mais simples que NKRO!)
  BootKeyboard.begin();
  ...
  Serial.println("READY:BOOTKEYBOARD-ABSOLUTEMOUSE");
  Serial.flush();
...
void handleMoveRelative(String coords) {
  ...
  // Movimento RELATIVO usando Mouse.move()
  Mouse.move(dx, dy);
...
// ============================================================================
// COMANDOS DE TECLADO - USANDO BOOTKEYBOARD!
// ============================================================================
...
void handleKeyPress(String key) {
  ...
  BootKeyboard.write(keyChar);  // ← BOOTKEYBOARD
...
void handleKeyDown(String key) {
  ...
  if (key.equalsIgnoreCase("alt") || key.equalsIgnoreCase("lalt")) {
    ...
    Serial.println("'");
    char keyChar = key.charAt(0);
    BootKeyboard.press(keyChar);  // ← BOOTKEYBOARD
  }
...
void handleKeyUp(String key) {
  ...
  // ✅ SOLTAR TECLAS COM BOOTKEYBOARD
  if (key.equalsIgnoreCase("alt") || key.equalsIgnoreCase("lalt")) {
    ...
    Serial.println("'");
    char keyChar = key.charAt(0);
    BootKeyboard.release(keyChar);  // ← BOOTKEYBOARD
  }
