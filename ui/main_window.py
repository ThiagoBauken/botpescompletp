...
class FishingBotUI:
    ...
    def __init__(self, config_manager=None, ws_client=None):
        ...
        self.arduino_baud_var = tk.StringVar(value="115200")
    ...
    def _initialize_engines(self):
        ...
        try:
            if CORE_ENGINES_AVAILABLE:
                ...
                # ✅ ARDUINO SEMPRE HABILITADO - SEM FALLBACK PYAUTOGUI
                use_arduino = True  # FORÇADO - NUNCA USAR PYAUTOGUI

                # 🔍 AUTO-DETECÇÃO: Procurar portas COM com Arduino
                detected_arduino_port = None
                saved_port = self.config_manager.get('arduino_port', 'auto')

                if use_arduino:  # Sempre True
                    try:
                        import serial.tools.list_ports

                        # 1️⃣ Tentar porta salva primeiro (se não for "auto")
                        if saved_port and saved_port != 'auto':
                            print(f"  🔍 Tentando porta salva: {saved_port}...")
                            try:
                                # Verificar se porta existe
                                available_ports = [p.device for p in serial.tools.list_ports.comports()]
                                if saved_port in available_ports:
                                    detected_arduino_port = saved_port
                                    print(f"  ✅ Usando porta salva: {saved_port}")
                                else:
                                    print(f"  ⚠️ Porta salva {saved_port} não disponível, detectando...")
                            except Exception as e:
                                print(f"  ⚠️ Erro ao verificar porta salva: {e}")

                        # 2️⃣ Se porta salva não funcionou, fazer auto-detecção
                        if not detected_arduino_port:
                            print("  🔍 Auto-detectando Arduino nas portas COM...")
                            for port in serial.tools.list_ports.comports():
                                if 'Arduino' in port.description or 'CH340' in port.description or 'USB' in port.description:
                                    detected_arduino_port = port.device
                                    print(f"  ✅ Arduino detectado: {detected_arduino_port} ({port.description})")
                                    break

                        # 3️⃣ SALVAR porta detectada no config (SE MUDOU)
                        if detected_arduino_port and detected_arduino_port != saved_port:
                            print(f"  💾 Salvando porta: {detected_arduino_port} → data/config.json")
                            self.config_manager.set('arduino_port', detected_arduino_port)
                            try:
                                self.config_manager.save_user_config()
                                print(f"  ✅ Porta salva! Próximo boot usará {detected_arduino_port}")
                            except Exception as e:
                                print(f"  ⚠️ Erro ao salvar: {e}")

                        if not detected_arduino_port:
                            print("  ⚠️ Nenhum Arduino detectado")
                            print("  ℹ️ Conecte o Arduino e use a aba Arduino")
                    except Exception as e:
                        print(f"  ⚠️ Erro ao detectar Arduino: {e}")

                # ✅ SEMPRE USAR ARDUINO - SEM FALLBACK!
                print("  🤖 Modo Arduino HID OBRIGATÓRIO")
                try:
                    ...
                    if detected_arduino_port:
                        print(f"  ✅ ArduinoInputManager criado para {detected_arduino_port}")
                        ...
                        try:
                            if hasattr(self.input_manager, 'connect_arduino'):
                                ...
                                if success:
                                    ...
                                    print(f"  ⚠️ Falha na conexão automática. Use a aba Arduino para conectar manualmente.")
                        except Exception as e:
                            print(f"  ⚠️ Erro na conexão automática: {e}")
                            print(f"  ℹ️ Use a aba Arduino para conectar manualmente.")
                    else:
                        print("  ⚠️ ArduinoInputManager criado (sem Arduino detectado)")
                        print("  ℹ️ Conecte o Arduino e use a aba Arduino para conectar")

                    print("  🔒 BOT FUNCIONA APENAS COM ARDUINO USB HID")
                except ImportError as e:
                    print(f"  ❌ ERRO CRÍTICO: ArduinoInputManager não disponível: {e}")
                    print(f"  ❌ Bot NÃO pode funcionar sem Arduino!")
                    self.input_manager = None
                except Exception as e:
                    print(f"  ❌ ERRO ao inicializar Arduino: {e}")
                    import traceback
                    traceback.print_exc()
                    self.input_manager = None
    ...
    def open_game_visualizer(self):
        ...
        try:
            if hasattr(self, 'game_visualizer') and self.game_visualizer:
                ...
                messagebox.showwarning(_("messages.title_warning"), _("messages.viewer_not_available"))
        except Exception as e:
            print(f"[ERROR] Erro ao abrir visualizador: {e}")
            messagebox.showerror(_("messages.title_error"), _("messages.error_opening_viewer").format(error=e))
    ...
    def setup_ui_components(self):
        ...
        try:
            ...
            # ✅ AUTO-REFRESH: Atualizar portas Arduino automaticamente após UI pronta
            # Usar after() para garantir que todos os widgets estejam criados
            self.main_window.after(100, self._auto_refresh_arduino_on_startup)
    ...
    def animate_gif(self):
        ...
        try:
            ...
            # Se bot está rodando, anima; se não, fica parado no frame atual
            if self.gif_is_animating:
                # Próximo frame (loop circular)
                self.current_gif_frame = (self.current_gif_frame + 1) % len(self.gif_frames)

            # Atualizar ambos os labels (esquerdo e direito)
            current_frame = self.gif_frames[self.current_gif_frame]
            if hasattr(self, 'gif_label_left'):
                self.gif_label_left.config(image=current_frame)
            if hasattr(self, 'gif_label_right'):
                self.gif_label_right.config(image=current_frame)

            # Agendar próximo frame
            duration = self.gif_durations[self.current_gif_frame] if self.gif_is_animating else 100
            self.main_window.after(duration, self.animate_gif)

        except Exception as e:
            print(f"[WARN] Erro na animação do GIF: {e}")

    def start_gif_animation(self):
        """Iniciar animação do GIF quando bot começar"""
        self.gif_is_animating = True

    def stop_gif_animation(self):
        """Parar animação do GIF quando bot parar"""
    ...
    def create_interface(self):
        ...
        try:
            ...
            # Frame horizontal para GIF + Título + GIF
            title_frame = tk.Frame(main_frame, bg=self.theme_colors['bg_primary'])
            title_frame.pack(pady=15)

            # GIF animado à esquerda
            self.gif_label_left = tk.Label(title_frame, bg=self.theme_colors['bg_primary'])
            self.gif_label_left.pack(side='left', padx=(0, 10))

            # Título com cores melhoradas - FISHING MAGEBOT (no centro)
            title_label = tk.Label(title_frame,
                                 text="Fishing MageBOT",
    ...
    def create_config_tab(self):
        ...
        macro_combo = tk.OptionMenu(chest_grid, self.macro_type_var,
                                     _("config_hardcoded.macro_type_default"),
                                     _("config_hardcoded.macro_type_custom"))
    ...
    def create_feeding_tab(self):
        ...
        tk.Label(status_frame, text=_("feeding_hardcoded.sistema_idêntico_ao"),
    ...
    def reset_template_confidence(self):
        ...
        try:
            if hasattr(tk, 'messagebox') and tk.messagebox.askyesno(_("messages.title_reset"), _("messages.reset_confidence_confirm")):
                ...
                default_values = {
                    ...
                    # Iscas/Carnes
                    'carneurso': 0.7, 'carnedelobo': 0.74, 'crocodilo': 0.7, 'bigcat': 0.7, 'grub': 0.6, 'minhoca': 0.74,
                    # Alimentação
    ...
    def save_current_as_default(self):
        ...
        try:
            if hasattr(tk, 'messagebox') and tk.messagebox.askyesno(
                _("messages.title_confirm"),
                _("messages.save_as_default_confirm")
            ):
                import json
                import os
                ...
                if os.path.exists(user_config_path):
                    ...
                    tk.messagebox.showinfo(_("messages.title_success"), _("messages.settings_saved_as_default"))
                else:
                    print(f"[ERROR] Arquivo de configuração do usuário não encontrado: {user_config_path}")
                    tk.messagebox.showerror(_("messages.title_error"), _("messages.config_file_not_found"))

        except Exception as e:
            print(f"[ERROR] Erro ao salvar como padrão: {e}")
            tk.messagebox.showerror(_("messages.title_error"), _("messages.error_saving_default").format(error=e))
    ...
    def create_anti_detection_tab(self):
        ...
        tk.Checkbutton(click_frame, text=_("ui_hardcoded.ativar_variacao_cliques"),
        ...
        info_label = tk.Label(s_key_frame,
                             text=_("anti_detection_hardcoded.info_ciclo_movements"),
        ...
        # Usar a variável já definida no __init__ (linha 211)
        tk.Checkbutton(breaks_frame, text=_("ui_hardcoded.ativar_pausas_naturais"),
                      variable=self.natural_breaks_var,
                      bg='#2a2a2a', fg='white', selectcolor='#2a2a2a',
                      command=self.update_anti_detection_settings).pack(pady=5)
    ...
    def save_anti_detection_config(self):
        ...
        try:
            ...
            print(f"[ERROR] Erro ao salvar configurações: {e}")
            messagebox.showerror(_("messages.title_error"), _("messages.error_saving").format(error=e))
    ...
    def create_catch_viewer_tab(self):
        ...
        resolution_combo = ttk.Combobox(resolution_frame, textvariable=self.capture_resolution_var,
                                       values=['1920x1080', '1366x768', '1280x720', '1600x900',
                                               _("ui_hardcoded.fullscreen")],
...
👁️ Use 'Abrir Janela CATCH' para visualizar as detecções."""
    ...
    def create_hotkeys_tab(self):
        ...
        main_hotkeys = {
            'start': _('help_hardcoded.hotkey_start_bot'),
            'pause': _('help_hardcoded.hotkey_pause_resume'),
            'stop': _('help_hardcoded.hotkey_stop_bot'),
            'emergency': _('help_hardcoded.hotkey_emergency_stop')
        }
        ...
        macro_hotkeys = {
            'macro_execute': _('help_hardcoded.hotkey_execute_macro'),
            'macro_chest': _('help_hardcoded.hotkey_chest_macro'),
            'macro_record': _('help_hardcoded.hotkey_record_macro')
        }
        ...
        test_hotkeys = {
            'test_mouse': _('help_hardcoded.hotkey_test_mouse'),
            'test_feeding': _('help_hardcoded.hotkey_test_feeding'),
            'test_cleaning': _('help_hardcoded.hotkey_test_cleaning')
        }
    ...
    def create_arduino_tab(self):
        ...
        info_text = _("help_hardcoded.arduino_info_text")
    ...
    def create_help_tab(self):
        ...
        instructions = [
            _("help_hardcoded.help_instruction_1"),
            _("help_hardcoded.help_instruction_2"),
            _("help_hardcoded.help_instruction_3"),
            _("help_hardcoded.help_instruction_4"),
            _("help_hardcoded.help_instruction_5")
        ]
        ...
        troubleshooting = [
            _("help_hardcoded.help_trouble_1"),
            _("help_hardcoded.help_trouble_2"),
            _("help_hardcoded.help_trouble_3"),
            _("help_hardcoded.help_trouble_4"),
            _("help_hardcoded.help_trouble_5")
        ]
    ...
    def _load_hotkeys_from_config(self):
        """Carregar hotkeys salvas e aplicar nas variáveis da UI"""
        try:
            if hasattr(self, 'config_manager') and self.config_manager and hasattr(self, 'hotkey_vars'):
                saved_hotkeys = self.config_manager.get('hotkeys', {})
                if saved_hotkeys:
                    print(f"[HOTKEY] Carregando hotkeys salvas: {saved_hotkeys}")
                    # Aplicar cada hotkey salva nas variáveis da UI
                    for hotkey_name, hotkey_value in saved_hotkeys.items():
                        if hotkey_name in self.hotkey_vars:
                            self.hotkey_vars[hotkey_name].set(hotkey_value)
                            print(f"[HOTKEY] {hotkey_name} = {hotkey_value}")
                    print("[OK] Hotkeys aplicadas na interface")
                else:
                    print("[INFO] Nenhuma hotkey salva encontrada, usando padrões")
        except Exception as e:
            print(f"[ERROR] Erro ao carregar hotkeys na UI: {e}")
    ...
    def load_hotkeys_config(self):
        ...
        try:
            ...
            defaults = {
                ...
                'test_feeding': 'f6',
                'test_cleaning': 'f5',
                'test_maintenance': 'page down'
            }

            # Carregar do ConfigManager se disponível
            if hasattr(self, 'config_manager') and self.config_manager:
                saved_hotkeys = self.config_manager.get('hotkeys', {})
                if saved_hotkeys:
                    print(f"[HOTKEY] Carregando hotkeys salvas: {saved_hotkeys}")
                    # Mesclar com defaults (usar salvo se existir, senão usar padrão)
                    for key in defaults:
                        if key in saved_hotkeys:
                            defaults[key] = saved_hotkeys[key]
                    return defaults

            return defaults
        except Exception as e:
            print(f"[ERROR] Erro ao carregar hotkeys: {e}")
            return {
                'start': 'f9',
                'pause': 'f2',
                'stop': 'f1',
    ...
    def save_hotkeys_config(self):
        ...
        try:
                ...
                if hasattr(self.config_manager, 'save_config'):
                    ...
                    if hasattr(self, 'hotkey_manager') and self.hotkey_manager:
                        ...
                        if self.hotkey_manager.reload_hotkeys():
                            ...
                            messagebox.showinfo(_("messages.title_success"), _("messages.hotkeys_saved_active"))
                        else:
                            print("[WARN] Erro ao recarregar hotkeys (reinicie o bot)")
                            messagebox.showinfo(_("messages.title_success"), _("messages.hotkeys_saved_restart"))
                    else:
                        messagebox.showinfo(_("messages.title_success"), _("messages.hotkeys_saved_restart"))
            ...
            print(f"[ERROR] Erro ao salvar: {e}")
            messagebox.showerror(_("messages.title_error"), _("messages.error_saving_hotkeys").format(error=e))
            if hasattr(self, 'hotkey_status_label'):
                self.hotkey_status_label.config(
        ...
        try:
            if self.catch_viewer_window is None or not self.catch_viewer_window.winfo_exists():
                ...
                self.viewer_fishing_stats = tk.Label(fishing_frame,
                    text=_("ui_hardcoded.fishing_stats_initial"),
                    font=('Consolas', 9), fg='white', bg='#2a2a2a', justify='left')
    ...
    def catch_viewer_loop(self):
        ...
        try:
            ...
            template_groups = {
                ...
                'meat_bait': ['grub', 'minhoca', 'carneurso', 'carnedelobo', 'crocodilo', 'bigcat'],
                                ...
                                try:
                                    ...
                                    if hasattr(self, 'viewer_rod_status'):
                                        try:
                                            ...
                                            if inventory_open:
                                                rod_text = _("ui_hardcoded.rod_status_inventory_open")
                                                rod_text += _("ui_hardcoded.detailed_analysis_active")
                                            else:
        ...
        try:
            ...
            if I18N_AVAILABLE:
                ...
                if selected_language == 'en':
                    ...
                    message = f"Interface language changed to {lang_names.get(selected_language)}!\n\nTab names and menus have been updated.\nFor full translation of all UI elements, please restart the application."
                elif selected_language == 'es':
                    title = "Idioma Cambiado"
                    message = f"Idioma de la interfaz cambiado a {lang_names.get(selected_language)}!\n\nLos nombres de pestañas y menús se han actualizado.\nPara la traducción completa de todos los elementos, reinicie la aplicación."
                elif selected_language == 'ru':
                    title = "Язык Изменён"
                    message = f"Язык интерфейса изменён на {lang_names.get(selected_language)}!\n\nНазвания вкладок и меню обновлены.\nДля полного перевода всех элементов перезапустите приложение."
                else:  # pt
                    title = "Idioma Alterado"
                    message = f"Idioma da interface alterado para {lang_names.get(selected_language)}!\n\nNomes das abas e menus foram atualizados.\nPara tradução completa de todos os elementos, reinicie a aplicação."
    ...
    def save_feeding_config(self):
        ...
        try:
            ...
            messagebox.showerror(_("messages.title_error"), _("messages.error_saving_feeding").format(error=e))
    ...
    def load_anti_detection_config(self):
        ...
        try:
            if hasattr(self, 'config_manager') and self.config_manager:
                ...
                if min_delay_ms and isinstance(min_delay_ms, list) and len(min_delay_ms) >= 2:
                    if hasattr(self, 'click_delay_min_var'):
                        self.click_delay_min_var.set(str(min_delay_ms[0]))
                        print(f"[DOC] click_delay_min carregado: {min_delay_ms[0]}ms")
                    if hasattr(self, 'click_delay_max_var'):
                ...
                if natural_breaks is not None and hasattr(self, 'natural_breaks_var'):
                    self.natural_breaks_var.set(natural_breaks)
                    print(f"[DOC] natural_breaks carregado: {natural_breaks}")

                # Carregar break mode
                break_mode = self.config_manager.get('anti_detection.break_mode')
                if break_mode and hasattr(self, 'break_mode_var'):
                    self.break_mode_var.set(break_mode)
                    print(f"[DOC] break_mode carregado: {break_mode}")

                # Carregar break catches
                break_catches = self.config_manager.get('anti_detection.break_catches')
                if break_catches is not None and hasattr(self, 'break_catches_var'):
                    self.break_catches_var.set(str(break_catches))
                    print(f"[DOC] break_catches carregado: {break_catches}")

                # Carregar break minutes
                break_minutes = self.config_manager.get('anti_detection.break_minutes')
                if break_minutes is not None and hasattr(self, 'break_minutes_var'):
                    self.break_minutes_var.set(str(break_minutes))
                    print(f"[DOC] break_minutes carregado: {break_minutes}")

                # Carregar movement durations
                movement_a_min = self.config_manager.get('anti_detection.movement_duration_a_min')
                if movement_a_min is not None and hasattr(self, 'movement_duration_a_min_var'):
                    self.movement_duration_a_min_var.set(str(movement_a_min))
                    print(f"[DOC] movement_duration_a_min carregado: {movement_a_min}")

                movement_a_max = self.config_manager.get('anti_detection.movement_duration_a_max')
                if movement_a_max is not None and hasattr(self, 'movement_duration_a_max_var'):
                    self.movement_duration_a_max_var.set(str(movement_a_max))

                movement_d_min = self.config_manager.get('anti_detection.movement_duration_d_min')
                if movement_d_min is not None and hasattr(self, 'movement_duration_d_min_var'):
                    self.movement_duration_d_min_var.set(str(movement_d_min))

                movement_d_max = self.config_manager.get('anti_detection.movement_duration_d_max')
                if movement_d_max is not None and hasattr(self, 'movement_duration_d_max_var'):
                    self.movement_duration_d_max_var.set(str(movement_d_max))

                # ✅ Carregar configurações de ciclo de tecla S (se existir)
                if hasattr(self, 's_key_cycle_enabled'):
                    ...
                    if s_hold_min is not None and hasattr(self, 's_hold_duration_min'):
                    ...
                    if s_hold_max is not None and hasattr(self, 's_hold_duration_max'):
                    ...
                    if s_release_min is not None and hasattr(self, 's_release_duration_min'):
                    ...
                    if s_release_max is not None and hasattr(self, 's_release_duration_max'):
                        self.s_release_duration_max.set(float(s_release_max))

                print("[OK] Configurações de anti-detecção carregadas completamente")
            else:
                print("[WARN] ConfigManager não disponível para carregar configurações")
        except Exception as e:
    ...
    def _on_fishing_error(self, error_message):
        ...
        try:
            ...
            if "crítico" in error_message.lower():
                messagebox.showerror(_("messages.title_critical_error"), error_message)
    ...
    def _auto_refresh_arduino_on_startup(self):
        """
        Atualizar portas Arduino automaticamente ao iniciar o programa.
        Simula o clique no botão "Atualizar" da aba Arduino.
        """
        try:
            print("[STARTUP] 🔄 Atualizando portas Arduino automaticamente...")

            # 1️⃣ Atualizar lista de portas COM na UI
            self.refresh_arduino_ports()

            # 2️⃣ Verificar se Arduino já foi conectado durante _initialize_engines()
            if self.input_manager and hasattr(self.input_manager, 'connected') and self.input_manager.connected:
                # Arduino já conectado durante inicialização
                connected_port = self.config_manager.get('arduino_port', 'auto')
                self.log_arduino(f"✅ Arduino já conectado automaticamente: {connected_port}")
                self.log_arduino(f"🎮 Bot pronto para uso!")

                # Atualizar status na UI
                if hasattr(self, 'arduino_connection_status'):
                    self.arduino_connection_status.config(text=_("ui_hardcoded.conectado_e_funcionando"))
                if hasattr(self, 'arduino_status_indicator'):
                    self.arduino_status_indicator.config(fg="green")

            else:
                # Arduino não conectado ainda
                detected_port = self.config_manager.get('arduino_port', 'auto')

                if detected_port and detected_port != 'auto':
                    # Porta foi detectada mas não conectou - tentar conectar agora
                    self.log_arduino(f"🔌 Tentando conectar Arduino em {detected_port}...")

                    # Atualizar dropdown para mostrar porta detectada
                    if hasattr(self, 'arduino_port_var'):
                        self.arduino_port_var.set(detected_port)

                    # Tentar conectar automaticamente
                    self.main_window.after(500, lambda: self.connect_arduino_auto(detected_port))
                else:
                    # Nenhum Arduino detectado
                    self.log_arduino(f"⚠️ Nenhum Arduino detectado nas portas COM")
                    self.log_arduino(f"ℹ️ Conecte o Arduino e clique em 'Atualizar' ou 'Conectar'")

            print("[STARTUP] ✅ Auto-refresh Arduino concluído")

        except Exception as e:
            print(f"[STARTUP] ❌ Erro no auto-refresh: {e}")
            import traceback
            traceback.print_exc()

    def connect_arduino_auto(self, port):
        """Conectar Arduino automaticamente (chamado por after())"""
        try:
            if hasattr(self, 'arduino_port_var'):
                self.arduino_port_var.set(port)

            # Simular clique no botão conectar
            if hasattr(self, 'connect_to_arduino'):
                self.connect_to_arduino()
        except Exception as e:
            self.log_arduino(f"❌ Erro na conexão automática: {e}")
    ...
    def _sync_config_to_server(self):
        """Sincronizar configurações com servidor via WebSocket"""
        try:
            if not self.ws_client:
                print("[WARN] WebSocket não conectado - sincronização ignorada")
                return

            # Importar função de sincronização
            from client.server_connector import _sync_config_with_server

            print("[SYNC] Sincronizando configurações com servidor...")
            _sync_config_with_server(self.ws_client)
            print("[OK] Sincronização concluída!")

        except Exception as e:
            print(f"[ERROR] Erro ao sincronizar com servidor: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _auto_connect_arduino(self):
        ...
        try:
            # ✅ ARDUINO SEMPRE HABILITADO - NÃO VERIFICAR CONFIG
            use_arduino = True  # FORÇADO - SEM FALLBACK
            ...
            if use_arduino and not self.arduino_connected:
                ...
                if not use_arduino:
                    # Esta mensagem NUNCA deve aparecer agora
        ...
        try:
            ...
            try:
                ...
                if windows:
                    ...
                    messagebox.showinfo(_("messages.title_success"), _("messages.window_detected").format(title=title, x=x, y=y, width=width, height=height))
                else:
                    print("[ERROR] Nenhuma janela do Rust encontrada")
                    messagebox.showwarning(_("messages.title_warning"), _("messages.rust_window_not_found"))

            except ImportError:
                print("[WARN] win32gui não disponível, usando detecção manual")
                messagebox.showinfo(_("messages.title_info"), _("messages.auto_detection_unavailable"))

        except Exception as e:
            print(f"[ERROR] Erro ao detectar janela: {e}")
            messagebox.showerror(_("messages.title_error"), _("messages.error_detecting_window").format(error=e))
