...
try:
    ...
    print("[WARN] i18n not available in license_dialog")
...
class LicenseDialog:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(
            main_frame,
            text=_("license.activation_title"),
            font=('Arial', 16, 'bold'),
            fg='#ffffff',
            bg='#2d2d2d'
        )
...
Por favor, insira sua chave de licença abaixo:"""
        ...
        tk.Label(
            key_frame,
            text=_("license.license_key_label"),
            font=('Arial', 10, 'bold'),
            fg='#ffffff',
            bg='#2d2d2d'
        ).pack(anchor='w')
...
• Mantenha sua chave segura"""
    ...
    def activate_license(self):
        ...
        if not license_key:
            messagebox.showerror(_("messages.title_error"), _("license.enter_key_required"))
            return
        
        # Desabilitar botão e mostrar status
        self.activate_btn.config(state='disabled')
        self.status_label.config(text=_("license.activating"))
        self.root.update()
    ...
    def handle_activation_result(self, success, message, license_key):
        ...
        if success:
            self.status_label.config(text=_("license.activated_success"), fg='#28a745')
...
⏰ Tempo restante: {info.get('time_remaining', 'N/A')} segundos"""
            ...
            info_text = _("license.not_licensed")
