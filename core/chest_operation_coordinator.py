...
class ChestOperationCoordinator:
    ...
    def __init__(self, config_manager, template_engine=None, feeding_system=None, rod_maintenance_system=None, inventory_manager=None, input_manager=None, ws_client=None, on_batch_complete=None):
        ...
        self.on_batch_complete = on_batch_complete  # ✅ NOVO: Callback para notificar FishingEngine quando batch completar
        ...
        # ORDEM CORRETA: 1. Alimentação → 2. Manutenção → 3. Limpeza
        self.priorities = {
            OperationType.FEEDING: 1,        # ✅ PRIMEIRO: Alimentação
            OperationType.MAINTENANCE: 2,    # ✅ SEGUNDO: Manutenção de varas
            OperationType.CLEANING: 3        # ✅ TERCEIRO: Limpeza do inventário
        }
        ...
        try:
            with self.execution_lock:
                ...
                if maintenance_in_queue:
                    ...
                    time.sleep(0.3)  # ✅ REDUZIDO: 1.0s → 0.3s (mais rápido)
    ...
    def _open_chest(self) -> bool:
        ...
        try:
            ...
            if self.input_manager and hasattr(self.input_manager, 'key_down'):
                _safe_print(f"   🚀 Enviando key_down('ALT') via {type(self.input_manager).__name__}...")
                result = self.input_manager.key_down('ALT')
                _safe_print(f"   📥 Resultado: {result}")
                _safe_print("   ✅ ALT pressionado via InputManager")
            else:
                _safe_print("   ⚠️ InputManager não disponível - usando PyAutoGUI")
                pyautogui.keyDown('alt')
                _safe_print("   ⚠️ ALT pressionado via PyAutoGUI (fallback)")

            _safe_print("   ⏳ Aguardando 0.8s para Arduino processar...")
            ...
            if delta_x != 0 or dy != 0:
                if self.input_manager and hasattr(self.input_manager, 'move_camera_windows_style'):
                    _safe_print(f"   🚀 Chamando move_camera_windows_style({delta_x}, {dy}, steps=10)...")
                    result = self.input_manager.move_camera_windows_style(delta_x, dy, steps=10)
                    _safe_print(f"   📥 Resultado: {result}")
                    _safe_print("   ✅ Câmera movida via InputManager!")
                else:
                    # Fallback para API Windows se Arduino não disponível
                    _safe_print("   ⚠️ InputManager não tem move_camera_windows_style - usando _camera_turn_in_game")
                    self._camera_turn_in_game(delta_x, dy)
                    _safe_print("   ⚠️ Câmera movida (fallback)")

            _safe_print("   ⏳ Aguardando 0.3s...")
            time.sleep(0.3)
            _safe_print("   ✅ Delay concluído\n")

            # PASSO 5: Pressionar E via Arduino (se disponível)
            _safe_print("\n[5/5] === PRESSIONANDO TECLA E (INTERAGIR) ===")
            _safe_print(f"   📊 InputManager tem press_key? {hasattr(self.input_manager, 'press_key') if self.input_manager else False}")
            ...
            if self.input_manager and hasattr(self.input_manager, 'press_key'):
                ...
                _safe_print(f"   📥 Resultado: {result}")
                _safe_print("   ✅ E pressionado via InputManager")
            else:
                _safe_print("   ⚠️ InputManager não disponível - usando PyAutoGUI")
                pyautogui.press('e')
                _safe_print("   ⚠️ E pressionado via PyAutoGUI (fallback)")

            _safe_print("   ⏳ Aguardando 0.3s para baú abrir...")
    ...
    def _check_pair_switch_needed_after_chest(self) -> bool:
        ...
        No modo HÍBRIDO, servidor decide quando trocar par via session.should_switch_rod_pair()

        Returns:
            bool: Always False (decision now server-side)
        """
        try:
            # ✅ CORREÇÃO #8: Decisão agora é do SERVIDOR
            # O servidor rastreia rod_uses e decide via session.should_switch_rod_pair()
            # Quando necessário, servidor envia "request_rod_analysis" → maintenance sequence
            _safe_print("   ℹ️ [HÍBRIDO] Decisão de troca de par controlada pelo servidor")
            return False

            # ❌ REMOVIDO: Lógica local de decisão (conflita com servidor)
            # if not self.rod_maintenance_system:
            #     return False
            #
            # rod_manager = getattr(self.rod_maintenance_system, 'rod_manager', None)
    ...
    def _remove_rod_from_hand_before_chest(self) -> int:
        ...
        try:
            ...
            _safe_print(f"\n🔍 [COORDINATOR] === DETECTANDO VARA ATUAL ===")
            current_rod = rod_manager.get_current_rod()
            _safe_print(f"   📊 get_current_rod() retornou: {current_rod}")
            _safe_print(f"   📊 Par atual: {rod_manager.rod_pairs[rod_manager.current_pair_index]}")
            _safe_print(f"   📊 Posição no par: {rod_manager.current_rod_in_pair}")

            if current_rod:
                _safe_print(f"   🎣 Vara {current_rod} detectada - removendo...")

                # Remover vara usando o método do rod_manager
                _safe_print(f"   🚀 Chamando rod_manager.remove_rod_from_hand({current_rod})...")
                success = rod_manager.remove_rod_from_hand(current_rod)
                _safe_print(f"   📥 Resultado: {success}\n")
